...
def test_dummy(args,test_dataset,modelD,net,dummy,experiment,\
    ...
    test_loader = DataLoader(test_dataset, num_workers=0, batch_size=604)
    correct = 0
...
def test_random(args,test_dataset,modelD,net,experiment,\
    ...
    test_loader = DataLoader(test_dataset, num_workers=0, batch_size=512)
    correct = 0
    preds_list, probs_list, labels_list = [], [],[]
    for p_batch in test_loader:
        p_batch_var = Variable(p_batch).cuda()
...
def train_random(args,modelD,train_dataset,test_dataset,\
    ...
    train_loader = DataLoader(train_dataset, num_workers=0, batch_size=3000)
...
def test_occupation(args,test_dataset,modelD,net,experiment,epoch,filter_set=None):
    test_loader = DataLoader(test_dataset, num_workers=0, batch_size=8000)
    correct = 0
    preds_list, labels_list, probs_list = [], [],[]
    for p_batch in test_loader:
        p_batch_var = Variable(p_batch).cuda()
...
def train_occupation(args,modelD,train_dataset,test_dataset,\
    ...
    opt = optimizer(net.parameters(),'adam', args.lr)
    train_loader = DataLoader(train_dataset, num_workers=0, batch_size=8000)
    train_data_itr = enumerate(train_loader)
    criterion = nn.NLLLoss()
...
def calc_attribute_bias(mode,args,modelD,experiment,\
    ...
    data_loader = DataLoader(dataset, num_workers=0, batch_size=4000)
    if args.show_tqdm:
...
def test(dataset, args, all_hash, modelD, subsample=1):
    l_ranks, r_ranks = [], []
    test_loader = DataLoader(dataset, num_workers=0, collate_fn=collate_fn)
...
def test_gcmc(dataset, args, modelD,filter_set=None):
    test_loader = DataLoader(dataset, batch_size=4000, num_workers=0, collate_fn=collate_fn)
    cst_inds = np.arange(args.num_ent, dtype=np.int64)[:,None]
    if args.show_tqdm:
        data_itr = tqdm(enumerate(test_loader))
    else:
...
def train_fairness_classifier_gcmc(train_dataset,args,modelD,experiment,fairD,\
    ...
    train_loader = DataLoader(train_dataset, num_workers=0, batch_size=8000)#, collate_fn=collate_fn)
    correct = 0
    total_ent = 0
...
def test_fairness_gcmc(test_dataset,args,modelD,experiment,fairD,\
    ...
    test_loader = DataLoader(test_dataset, num_workers=0, batch_size=8000)#, collate_fn=collate_fn)
    correct = 0
    total_ent = 0
    precision_list = []
    recall_list = []
...
def test_fairness_nce(dataset,args,modelD,experiment,fairD,\
    ...
    test_loader = DataLoader(dataset, num_workers=0, batch_size=4096, collate_fn=collate_fn)
    correct = 0
    total_ent = 0
    precision_list = []
    recall_list = []
